...
test("concurrent service processes elect one server", async () => {
  ...
  const registration = path.join(root, "state", "kito", "service-local.json")
  const port = await availablePort()
  const config = path.join(root, "config", "kito", "service-local.json")
  await fs.mkdir(path.join(root, "config", "kito"), { recursive: true })
...
test("unresponsive managed port occupancy reports a bounded conflict", async () => {
  ...
  await fs.writeFile(
    path.join(root, "config", "kito", "service-local.json"),
...
test("a foreign occupant of the channel default port falls back to an ephemeral port", async () => {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "opencode-service-ephemeral-"))
  ...
  try {
    const info = await waitForInfo(registration)
    expect(new URL(info.url).port).not.toBe(String(listener.port))
    expect(info.pid).toBe(owner.pid)
    await Effect.runPromise(Service.stop({ file: registration }).pipe(Effect.provide(NodeFileSystem.layer)))
    await owner.exited
  } finally {
