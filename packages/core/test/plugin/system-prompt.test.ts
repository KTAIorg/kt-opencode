...
describe("SystemPromptPlugin", () => {
  ...
  it.effect("selects model-lab prompts through session context hooks", () =>
    Effect.gen(function* () {
      ...
      const cases = [
        ["gpt-5", "You are Kito, You and the user share the same workspace"],
        ["gpt-4.1", "You are Kito, You and the user share the same workspace"],
        ["o3", "You are Kito, You and the user share the same workspace"],
  ...
  it.effect("selects against the catalog model ID instead of its alias", () =>
    Effect.gen(function* () {
      ...
      expect(physicalOpenAI.system[0]?.text).toContain("You are Kito, You and the user share the same workspace")
