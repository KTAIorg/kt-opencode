...
// etc.).
//
// The visual language mirrors the Kito app: the design tokens are a curated
// subset of the OC-2 semantic tokens in `packages/ui/src/styles/theme.css`, and
// the wordmark is the Kito ghost wordmark from
// `packages/ui/src/typography/wordmark/wordmark.tsx`.
...
export function success(options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider ? `Kito is now connected to ${escapeHtml(provider)}.` : "Kito is now authorized.",
...
export function error(detail: string, options?: CallbackPageOptions) {
  ...
  return renderDocument({
    ...
    body: renderCard({
      ...
      message: provider
        ? `Kito couldn't finish connecting to ${escapeHtml(provider)}.`
        : "Kito couldn't complete authorization.",
      detail,
      footnote: "Close this window and try again from Kito.",
    }),
  })
}
...
<html lang="en">
  <head>
    ...
    <title>${escapeHtml(input.title)} · Kito</title>
    <style>${STYLES}</style>
  </head>
  <body>
...
(function(){
  ...
  function fail(text){card.dataset.status="error";headline.textContent="Authorization failed";message.textContent=PROVIDER?("Kito couldn't finish connecting to "+PROVIDER+"."):"Kito couldn't complete authorization.";if(text){detail.textContent=text;detail.hidden=false}footnote.textContent="Close this window and try again from Kito."}
  function ok(){card.dataset.status="success";headline.textContent="Authorization successful";message.textContent=PROVIDER?("Kito is now connected to "+PROVIDER+"."):"Kito is now authorized.";detail.hidden=true;footnote.textContent="You can close this window.";setTimeout(function(){try{window.close()}catch(e){}},2500)}
  try{
...
// Kito ghost wordmark — inlined from packages/ui/src/typography/wordmark/wordmark.tsx.
const WORDMARK = `<svg class="wordmark" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 720 129" fill="none" aria-label="Kito" role="img">
        <g opacity="0.6" mask="url(#kito-ghost-mask)">
          <text x="360" y="96" text-anchor="middle" fill="currentColor" opacity="0.16" font-size="96" font-weight="600" font-family="ui-sans-serif, system-ui, sans-serif" letter-spacing="-0.04em">Kito</text>
        </g>
        <defs>
          <mask id="kito-ghost-mask" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="720" height="129">
            <rect width="720" height="129" fill="url(#kito-ghost-gradient)" />
          </mask>
          <linearGradient id="kito-ghost-gradient" x1="360" y1="68" x2="360" y2="129" gradientUnits="userSpaceOnUse">
