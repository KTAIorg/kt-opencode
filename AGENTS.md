...
- A password-protected local server needs `?auth_token=` on the app URL (`btoa("opencode:<password>")`, empty username becomes `opencode`). Titlebar Sign in/Top up calls `/ktai/account` on that same server.
