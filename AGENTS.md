...
- Identity for `/ktai/*` is `~/.local/share/kito/ktai-identity.json` (or `KTAI_IDENTITY_TOKEN` in embedded mode). A leftover 1.x `auth.json` token is not enough if Identity says it is expired; run Telegram login again. The managed NewAPI key stays in `ktai-api-key.json`.
...
- Telegram login is the first Kito auth method. After Identity succeeds, Kito calls NewAPI Ensure and persists the managed `name=kito` key locally (`ktai-api-key.json` under the Kito XDG data dir, `~/.local/share/kito`). Identity Bearer is never sent to NewAPI as a model key. On 401, `POST /ktai/ensure` re-issues that key.
