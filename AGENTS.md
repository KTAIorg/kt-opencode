...
- Apply `kt-input-correction` on every user message before acting. Default: spoken/typed `KD` means `KT` (including `KD settlement` → `kt-settlement`). Do not rewrite `KDC`. `Keto`/`Kete` means Kito. Glossary: `kt-input-correction/references/glossary.md` in `KTAIorg/kt-agent-skills`. Only skip a mapping when the user says they mean the literal form.
