...
- Top-up uses Identity-gated wallet routes (`GET /ktai/wallet/ktpay/info|pay|status`, `GET /ktai/wallet/deposit-address?chain=`). Fiat (WeChat/Alipay) and crypto both credit the KT Identity ledger. Deposit addresses are per `kt_account_id + chain`: TRC20 (`tron`) is USDT only; ERC20 (`ethereum`) is one address for both USDT and USDC. If ERC20 returns a TRON `T…` address, do not show that QR. Production `ktapi.cc` still 404s `/api/iam/ktpay/*` and deposit-address; wallet calls try `https://newapi-test.ktyun.cc` next. Do not point the whole server at test NewAPI or Kito models disappear. Identity itself has no wallet routes.
