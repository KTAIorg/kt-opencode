...
- This fork tracks upstream OpenCode **V2 beta** (`anomalyco/opencode` branch `beta`). The CLI binary is `opencode2`. Start the API with `bun dev serve --port 4106` from the repo root (or `opencode2 serve --port 4106`) and the app with `cd packages/app && bun dev -- --port 4456`. Do not reuse 4096/4444 if older Kito 1.x processes are already bound there, and do not restart existing app/server processes.
